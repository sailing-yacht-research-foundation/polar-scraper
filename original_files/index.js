...
function makeCert(
  ...
  return {
    ...
    hasPolars: hasPolars,
    polars: polars,
    hasTimeAllowances: timeAllowances,
...
async function scrapeAllCerts() {
  ...
  url =
    'https://www.regattaman.com/cert_list.php?yr=' +
    thisYear +
    '&lType=all&org=327&goback=%2Fcertificate_page.php%3Fcp_tab&ctyp=ORR&rnum=0&sort=0&ssort=0&sdir=true&ssdir=true';

  await page.goto(url, { timeout: 10000, waitUntil: 'load' });
  const orrFullUrls = await page.evaluate(() => {
    const results = [];
    document
      .querySelectorAll('#resTable-0 > tbody > tr > td > a')
      .forEach((btn) => {
        results.push(btn.onclick.toString().split('sku=')[1].split("'")[0]);
      });
    return results;
  });
  ...
  for (skuIndex in orrFullUrls) {
    url =
      'https://www.regattaman.com/cert_form.php?sku=' +
      orrFullUrls[skuIndex] +
      '&rnum=0&sort=undefined&ssort=undefined&sdir=true&ssdir=true';
    console.log(url);
    await page.goto(url, { timeout: 10000, waitUntil: 'load' });
    var certInfo = await page.evaluate(() => {
      const boatName = document
        .querySelector('#boat_name_sail')
        .textContent.split('   ')[0];
      const org = 'ORR';
      const suborg = 'None';
      ...
      document
        .querySelectorAll('#polar_speed > div > div > table > thead > tr > th')
        .forEach((d) => {
          if (!skippedFirst) {
            skippedFirst = true;
          } else {
      ...
      document
        ...
        .forEach((d) => {
          if (currentIndex >= 2 && currentIndex <= 11) {
            ...
            trueWindAngles.push({
              twa: parseFloat(d.textContent.replace('°', '')),
              speeds: speeds,
            });
          }
          currentIndex++;
      ...
      document
        .querySelectorAll('#polar_time > div > div > table > thead > tr > th')
        .forEach((d) => {
          if (!skippedFirst) {
            skippedFirst = true;
          } else {
    ...
    orrFullCerts.push(
      makeCert(
        ...
        certInfo.beamFt,
        certInfo.draftFt,
        certInfo.displacementKg,
        Buffer.from(extras).toString('base64'),
        true,
        certInfo.polars,
        true,
        certInfo.timeAllowances,
      ),
    );
  }
  ...
  var url =
    'https://www.regattaman.com/cert_list.php?yr=' +
    thisYear +
    '&lType=all&org=327&goback=%2Fcertificate_page.php%3Fcp_tab%3D0&ctyp=ORR-Ez%2CORR-Ez-SH%20Request%20Method:%20GET';
  console.log(url);
  ...
  const oneDesignUrls = await page.evaluate(() => {
    const urls = [];
    document
      .querySelectorAll('#odcert > tbody > tr > td > span > a')
      .forEach((c) => {
        urls.push(c.href);
      });
    return urls;
  ...
  const countryCodes1 = await page.evaluate(() => {
    ...
    const countries = [];
    document
      .querySelectorAll('#rms1 > table > tbody > tr > td.country > img')
      .forEach((c) => {
        codes.push(c.src.split('public/')[1].split('.gif')[0]);
      });
    document
      .querySelectorAll('#rms1 > table > tbody > tr > td.country > span')
      .forEach((c) => {
        countries.push(c.textContent);
      });
  ...
  const countryCodes3 = await page.evaluate(() => {
    ...
    const countries = [];
    document
      .querySelectorAll('#rms3 > table > tbody > tr > td.country > img')
      .forEach((c) => {
        codes.push(c.src.split('public/')[1].split('.gif')[0]);
      });
    document
      .querySelectorAll('#rms3 > table > tbody > tr > td.country > span')
      .forEach((c) => {
        countries.push(c.textContent);
      });
    return { countries, codes };
  ...
  const countryCodes5 = await page.evaluate(() => {
    ...
    const countries = [];
    document
      .querySelectorAll('#rms5 > table > tbody > tr > td.country > img')
      .forEach((c) => {
        codes.push(c.src.split('public/')[1].split('.gif')[0]);
      });
    document
      .querySelectorAll('#rms5 > table > tbody > tr > td.country > span')
      .forEach((c) => {
        countries.push(c.textContent);
      });
    return { countries, codes };
  ...
  const countryCodesAll = [
    { list: countryCodes1, index: { name: 'ORC Standard', value: '1' } },
    { list: countryCodes3, index: { name: 'Double Handed', value: '3' } },
    { list: countryCodes5, index: { name: 'Non Spinnaker', value: '5' } },
  ];
  ...
  for (countryListIndex in countryCodesAll) {
    ...
    for (countryIndex in list.countries) {
      ...
      axios({
        url:
          'https://data.orc.org/public/WPub.dll?action=DownRMS&CountryId=' +
          countryCode +
          '&ext=json&Family=' +
          familyCode +
          '&VPPYear=' +
  ...
  while (yearCounter >= 2017) {
    var url =
      'https://secure.headwaytechnology.com/crf.headwaydomain.com/page/validlist/?show=summary&filterby=all&sortby=lname&phyear=' +
      yearCounter +
      '&phkeyword=';
    console.log(url);
    await page.goto(url);
