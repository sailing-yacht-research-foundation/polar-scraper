  ...
  [
    {
      ...
      "environmentFiles": [
               {
                   "value": "arn:aws:s3:::syrf-dev-env-variables/polar-scraper.env",
                   "type": "s3"
               }
           ],
      "logConfiguration": {
...
resource "aws_iam_role" "polar_scraper_role" {
  name = "polar_scraper_iam"
...
resource "aws_iam_role_policy" "polar_scraper_role_policy" {
  ...
  role = aws_iam_role.polar_scraper_role.id
